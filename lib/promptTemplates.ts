...
export function buildExpansionInstructions(
  { roughDescription, motion, frameCount, rows, cols }: ExpandPromptRequest,
  transparentBackground = false,
): string {
  const motionLabel = MOTION_LABELS[motion];
  // OpenAI's gpt-image-1 is called with background:"transparent" for this path, so the
  // prompt should describe an empty background instead of asking for a keyable magenta
  // fill — requesting both confuses the model and defeats the native alpha channel.
  const backgroundRequirement = transparentBackground
    ? "Transparent background: no background elements, scenery, ground, or shadow of any kind — only the character sprite frames on empty space."
    : `Keyable background: flat, uniform solid background, pure magenta ${CHROMA_KEY_HEX}, no gradients, shadows, or texture on the background.`;
  return [
